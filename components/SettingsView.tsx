...
const SettingsView: React.FC = () => {
  ...
  const updateImageTemplate = (id: string, field: 'name' | 'prompt', value: string) => {
    setLocalSettings({
        ...localSettings,
        imageStyleTemplates: localSettings.imageStyleTemplates.map(t => 
  ...
  const removeImageTemplate = (id: string) => {
      setLocalSettings({
          ...localSettings,
          imageStyleTemplates: localSettings.imageStyleTemplates.filter(t => t.id !== id)
      });
  };
  ...
  const deleteModelConfig = (type: 'text' | 'image', id: string) => {
    ...
    // If we deleted the active one, reset to first available or empty
    if (type === 'text' && localSettings.activeTextModelId === id) {
       setLocalSettings(prev => ({ ...prev, activeTextModelId: prev.textModels[0]?.id || '' }));
    }
    if (type === 'image' && localSettings.activeImageModelId === id) {
       setLocalSettings(prev => ({ ...prev, activeImageModelId: prev.imageModels[0]?.id || '' }));
    }
  };

  const ModelConfigRow: React.FC<{ config: ModelConfig, type: 'text' | 'image', isActive: boolean }> = ({ config, type, isActive }) => {
    const isEditing = editingModelId === config.id;
      ...
      <div className={`bg-surface border ${isActive ? 'border-primary/50 bg-primary/5' : 'border-gray-800'} rounded-lg p-4 transition-all`}>
        <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-3">
                 <button 
                    onClick={() => setLocalSettings(prev => type === 'text' ? ({...prev, activeTextModelId: config.id}) : ({...prev, activeImageModelId: config.id}))}
            ...
            <div className="flex gap-2">
                <button onClick={() => setEditingModelId(isEditing ? null : config.id)} className="text-xs text-blue-400 hover:text-blue-300 underline">
                    {isEditing ? 'Done' : 'Edit'}
                </button>
                {!isActive && (
                    <button onClick={() => deleteModelConfig(type, config.id)} className="text-xs text-red-500 hover:text-red-400 hover:bg-red-900/20 p-1 rounded">
                        <Trash2 size={12} />
                    </button>
                )}
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4 animate-in fade-in slide-in-from-top-1">
                <div className="col-span-1 md:col-span-2">
                    ...
                    <input 
                        ...
                        value={config.name}
                        onChange={e => updateModelConfig(type, config.id, { name: e.target.value })}
                    />
                </div>
                <div>
                ...
                <div className="col-span-1 md:col-span-2">
                     ...
                     <div className="relative">
                        ...
                        <input 
                            ...
                            placeholder="sk-..."
                            value={config.apiKey}
                            onChange={e => updateModelConfig(type, config.id, { apiKey: e.target.value })}
                        />
                     </div>
                </div>
                    ...
                    <div>
                        ...
                        <div className="bg-surface border border-gray-800 rounded-lg divide-y divide-gray-800">
                            ...
                            <div className="p-4 flex items-center justify-between">
                                ...
                                <button 
                                    onClick={() => setLocalSettings({...localSettings, enableIntentAnalysis: !localSettings.enableIntentAnalysis})}
                        ...
                        <div className="space-y-6 max-w-4xl">
                            <div>
                                <div className="flex justify-between items-center mb-2">
                                    ...
                                    <button onClick={() => setLocalSettings({...localSettings, directorMainPrompt: DEFAULT_SETTINGS.directorMainPrompt})} className="text-xs text-gray-500 hover:text-white">Reset Default</button>
                                </div>
                                <div className="relative">
                                    <textarea 
                                        value={localSettings.directorMainPrompt}
                                        onChange={(e) => setLocalSettings({...localSettings, directorMainPrompt: e.target.value})}
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div>
                                        ...
                                        <textarea 
                                            ...
                                            onChange={(e) => setLocalSettings({...localSettings, intentPrompt: e.target.value})}
                                    ...
                                    <div>
                                        ...
                                        <textarea 
                                            ...
                                            onChange={(e) => setLocalSettings({...localSettings, editingPrompt: e.target.value})}
                                            className="w-full h-32 bg-black/50 border border-gray-700 rounded p-3 text-sm font-mono text-gray-300 focus:border-primary outline-none resize-none"
                                        />
                                    </div>
                                </div>
                        ...
                        <div className="max-w-4xl space-y-6">
                            ...
                            <div className="bg-surface border border-gray-800 rounded-lg p-6">
                                ...
                                <textarea 
                                    ...
                                    onChange={(e) => setLocalSettings({...localSettings, directorVar_director: e.target.value})}
                            ...
                            <div className="bg-surface border border-gray-800 rounded-lg p-6">
                                ...
                                <textarea 
                                    ...
                                    onChange={(e) => setLocalSettings({...localSettings, directorVar_storyboard: e.target.value})}
                            ...
                            <div className="bg-surface border border-gray-800 rounded-lg p-6">
                                ...
                                <textarea 
                                    ...
                                    onChange={(e) => setLocalSettings({...localSettings, directorVar_cinematography: e.target.value})}
                            ...
                            <div className="bg-surface border border-gray-800 rounded-lg p-6">
                                ...
                                <textarea 
                                    ...
                                    onChange={(e) => setLocalSettings({...localSettings, directorVar_continuity: e.target.value})}
